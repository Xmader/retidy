...
precedenceMap.forEach(function (tier, i) {
...
class FastPath {
    ...
    constructor(value: n.Node) {
        this.stack = [value]
    }

    // Static convenience function for coercing a value to a FastPath.
    static from(obj: FastPath | NodePath | n.Node) {
        ...
        if (obj instanceof types.NodePath) {
            ...
            for (var pp; (pp = (obj as NodePath).parentPath); obj = pp)
                stack.push((obj as NodePath).name, pp.value)
    ...
    call(callback: Function, ...names: string[]) {
        const s = this.stack
    ...
    each(callback: Function, ...names: string[]) {
        const s = this.stack
        const origLen = s.length
        let value = s[origLen - 1]
    ...
    map(callback: Function, ...names: string[]) {
        const s = this.stack
        const origLen = s.length
        let value = s[origLen - 1]
    ...
    needsParens(assumeExpressionContext?: boolean) {
        ...
        if (node.type === "AssignmentExpression" && (node as n.AssignmentExpression).left.type === "ObjectPattern") {
            return true
        }
        ...
        if ((node as n.Identifier).type === "Identifier") {
            return false
        }
        ...
        switch ((node as n.Node).type) {
            case "UnaryExpression":
            case "SpreadElement":
            case "SpreadProperty":
                return parent.type === "MemberExpression"
                    && name === "object"
                    && (parent as n.MemberExpression).object === node
            ...
            case "LogicalExpression":
                switch (parent.type) {
                    case "CallExpression":
                        return name === "callee"
                            && (parent as n.CallExpression).callee === node
                    ...
                    case "BinaryExpression":
                    case "LogicalExpression":
                        var po = (parent as n.LogicalExpression).operator
                        var pp = PRECEDENCE[po]
                        var no = (node as n.LogicalExpression).operator
                        ...
                        if (pp === np && name === "right") {
                            assert.strictEqual((parent as n.LogicalExpression).right, node)
                            return true
                        }
            ...
            case "Literal":
                return parent.type === "MemberExpression"
                    && isNumber.check((node as n.Literal).value)
                    && name === "object"
                    && (parent as n.MemberExpression).object === node
            ...
            case "ConditionalExpression":
                switch (parent.type) {
                    ...
                    case "NewExpression":
                        return name === "callee"
                            && (parent as n.CallExpression | n.NewExpression).callee === node

                    case "ConditionalExpression":
                        return name === "test"
                            && (parent as n.ConditionalExpression).test === node
            ...
            case "TSAsExpression":
                if (parent.type === "ArrowFunctionExpression" &&
                    name === "body" &&
                    (node as n.TSAsExpression).expression.type === "ObjectExpression") {
                    return true
                }
                break
            ...
            case "CallExpression":
                if (name === "declaration" &&
                    ...
                    n.FunctionExpression.check((node as n.CallExpression).callee)) {
                    return true
                }
        }
        ...
        if (parent.type === "NewExpression" &&
            name === "callee" &&
            (parent as n.NewExpression).callee === node) {
