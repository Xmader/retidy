...
export class PrintResult implements PrintResultType {
    ...
    warnedAboutToString: boolean = false
...
export class Printer {
    ...
    explicitTabWidth?: number

    constructor(config?: Options) {
        this.explicitTabWidth = config && config.tabWidth
    ...
    printGenerically(ast: types.ASTNode): PrintResultType {
        ...
        const oldReuseWhitespace = this.config.reuseWhitespace
    ...
    private _makePrintFunctionWith(options: any, overrides: any) {
        options = Object.assign({}, options, overrides)
        return (path: any) => this._print(path, options)
    }

    private _print(path: any, options: any) {
        ...
        if (options.includeComments) {
            return printComments(path, this._makePrintFunctionWith(options, {
        ...
        const lines = reprinter
            ...
            ? reprinter(this._print)
            : genericPrint(
                path,
                config,
                options,
                this._makePrintFunctionWith(options, {
                    includeComments: true,
                    avoidRootParens: false
                })
            )
...
function genericPrintNoParens(path: any, options: any, print: any) {
    ...
    switch (n.type) {
        ...
        case "ArrayPattern":
            ...
            var oneLine1 = joined.getLineLength(1) <= options.wrapColumn
            if (oneLine1) {
            ...
            path.each(function (elemPath: any) {
                ...
                if (!elem) {
                    ...
                    parts.push(lines)
                    if (i < len1 - 1 || (!oneLine1 && util.isTrailingCommaEnabled(options, "arrays")))
                        parts.push(",")
                    if (!oneLine1)
                        parts.push("\n")
                }
            ...
            if (oneLine1 && options.arrayBracketSpacing) {
                parts.push(" ]")
            } else {
                parts.push("]")
            }
        ...
        case "VariableDeclaration":
            ...
            var maxLen = 0
            var printed1 = path.map(function (childPath: any) {
            ...
            if (maxLen === 1) {
                parts.push(fromString(", ").join(printed1))
            } else if (printed1.length > 1) {
                parts.push(
                    fromString(",\n").join(printed1)
        ...
        case "ClassProperty":
            ...
            if (n.variance) {
                key1 = concat([printVariance(path, print), key1])
            }
        ...
        case "TupleTypeAnnotation":
            var printed2 = path.map(print, "types")
            var joined2 = fromString(", ").join(printed2)
            var oneLine2 = joined2.getLineLength(1) <= options.wrapColumn
            if (oneLine2) {
                if (options.arrayBracketSpacing) {
                    parts.push("[ ")
                } else {
            ...
            path.each(function (elemPath: any) {
                ...
                if (!elem) {
                    ...
                    parts.push(lines)
                    if (i < n.types.length - 1 || (!oneLine2 && util.isTrailingCommaEnabled(options, "arrays")))
                        parts.push(",")
                    if (!oneLine2)
                        parts.push("\n")
                }
            ...
            if (oneLine2 && options.arrayBracketSpacing) {
                parts.push(" ]")
            } else {
                parts.push("]")
            }
        ...
        case "TSTypeAnnotation": {
            ...
            var parent1 = path.getParentNode(0)
            let prefix = ": "
            if (namedTypes.TSFunctionType.check(parent1) || namedTypes.TSConstructorType.check(parent1)) {
                prefix = " => "
            }

            if (namedTypes.TSTypePredicate.check(parent1)) {
                prefix = " is "
            }
        ...
        case "TSTypeParameter":
            ...
            var parent2 = path.getParentNode(0)
        ...
        case "TSTypeAssertion":
            var withParens1 = n.extra && n.extra.parenthesized === true
            if (withParens1) {
                parts.push("(")
            }
